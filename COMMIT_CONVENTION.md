...
- 注释消毒迭代到不动点（`scripts/lib/markdown_sanitize.mjs`）：结果里不再残留注释起始符（`<` `!` `-` `-` 四个连续字符），任何 `##` 小节标题行都不会被删掉。因此正文里提到一次未配对的起始符，只会被拔掉标记本身，不再把中间的小节标题一起吃掉。代码围栏内的注释标记同样会被删除，这是有意为之：不变式要求返回值里不残留起始符，围着围栏开例外就等于给这条不变式开口子（方向偏严，不会放松判据）。
