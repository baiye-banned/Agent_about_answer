...
- 去掉 HTML 注释后不足 3 个字符，或内容正好是 `无` / `暂无` / `N/A` / `TODO` / `待补充` / `-` / `_No response_` 等占位文本，都算「空」；长度按码点计算，且必须含有汉字、字母或数字——纯 emoji、纯符号凑不出内容。
- 占位文本会被「规范化」后再判定：先去掉标点与列表符号（`-`、`1.`、`*`），再看剩下的是不是全由占位词拼成。因此 `暂无。`、`- 无`、`1. 无`、`_No response_`，以及 `- 无` + `- 待补充` 这类多行占位，都算「空」；而 `无 UI 变更` 这类含真实内容的写法不会被误判。
- 关联 issue 节写「无关联 issue」时，**短语之外还要写原因**（至少 3 个非占位实质字符），例如「无关联 issue，原因：补齐仓库协作模板」；只写短语本身判红。「无关联**的** issue」这类自然写法同样接受。
