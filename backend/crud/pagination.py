...
def seconds_text(column):
    ...
    退化成「同秒的行全部重来」或「同秒的行整批漏掉」。而列里存的**文本形状并不唯一**：
    - 服务端默认值（`CURRENT_TIMESTAMP` / MySQL 的 `DATETIME`）：`2026-09-22 10:09:21`；
    - 应用侧显式赋一个 Python datetime：SQLite 方言按固定格式绑成
      `2026-09-22 10:09:21.000000`，原样入库。生产代码今天不这样写 `updated_at`
      （列上只有 `server_default` / `onupdate` 两个服务端写入方），这个形状来自直接
      构造数据的调用方——本仓的 `tests/test_list_page_caps_191.py` 造同秒数据时即是一例。
