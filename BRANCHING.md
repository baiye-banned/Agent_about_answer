...
- Require status checks to pass：`main` 与 `develop` 都要加。**现状：两边都已设置，`strict=false`；`main` 9 条、`develop` 8 条。**
  ...
  - **漂移兜底（job 改名了怎么办）**：名单是按 **check-run 名逐字**匹配的，名字差一个字就等于永久 pending。因此**任何 job 改名，必须在同一个 PR 内把这里的名单一起改掉**（`main` 与 `develop` 都要）。改名后旧名**永不再产出**，该 PR 会**被平台直接阻断合入** —— 这不是需要靠人去发现的风险，**响亮失败本身就是兜底机制**：`develop` 侧的检查名漂移从此有报警（此前名单只在 `main` 生效，`develop` 上漂移没有任何机制会发现）。
  - 对账命令：`gh api repos/baiye-banned/Agent_about_answer/branches/develop/protection --jq '.required_status_checks.contexts[]'` 与 `gh pr checks <PR号>` 的输出**逐名**比对。
