...
一个基于 Vue 3 + FastAPI 的企业知识库智能问答平台。项目将登录鉴权、知识库管理、文件上传解析、Advanced RAG 检索、流式回答、多模态图片理解、会话记忆和 RAGAS 在线评估串成一条完整链路。
...
- 以 `retrieve_knowledge` 作为主检索链路，覆盖 RAG 路由判断、查询规划、关键词召回、向量召回、RRF 融合、rerank 和兜底检索。
...
| RAG / Retrieval | 查询规划、多路召回、RRF 融合、rerank |
...
7. `retrieve_knowledge()` 构建查询计划，并执行关键词召回、向量召回、RRF 融合和 rerank。
8. 检索出的 chunk 拼接为上下文，交给回答模型生成最终答案。
9. 最终答案通过 SSE 流式返回前端。
